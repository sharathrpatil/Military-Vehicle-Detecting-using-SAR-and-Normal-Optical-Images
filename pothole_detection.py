...
MODEL_PATHS = {
    ...
    "SAR Model": r"C:\SUHAS_P\defence pro\aircraft.pt"  # update if needed
}

from functools import lru_cache

@lru_cache(maxsize=2)
def load_model(model_type: str = "Normal Model"):
    path = MODEL_PATHS.get(model_type, MODEL_PATHS["Normal Model"])
    return YOLO(path)
...
def detect_from_image(image_path, model=None, model_type: str = "Normal Model"):
    if model is None:
...
def detect_from_video(video_path, model=None, model_type: str = "Normal Model"):
    if model is None:
        model = load_model(model_type)
